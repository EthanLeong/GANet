...
class OpenLaneDataset(TuSimpleDataset):
        ...
        for anno_list in data_list:
            with open(anno_list) as f:
                ...
                for line in lines:
                    ...
                    img_info.update(dict(anno_file=os.path.join('lane3d_1000_v1.2/lane3d_1000', raw_anno_file)))
    ...
    def load_labels(self, idx, offset_x, offset_y):
        anno_file = os.path.join(self.img_prefix, self.img_infos[idx]['anno_file'])
        lanes = []
        with open(anno_file, 'r') as anno_f:
            #lines = anno_f.readlines() # [],会有空的
            lines = json.load(anno_f)
        for lane in lines['lane_lines']:
            lane = lane['uv']
            coords = []
            for idx in range(len(lane[0])):
                coords.append(lane[0][idx] + offset_x)
                coords.append(lane[1][idx] + offset_y)
            if len(coords) > 3:
                lanes.append(coords)
    ...
    def evaluate(self, outputs, **eval_kwargs):
        ...
        gt_dir = os.path.join(self.data_root, 'lane3d_1000_v1.2/lane3d_1000')
        for idx in tqdm(range(len(self.img_infos))):
            raw_file = self.img_infos[idx]['raw_file']
            pr_anno = os.path.join(pr_dir,raw_file) 
            gt_anno = os.path.join(gt_dir,raw_file.replace('img/', "")) 
            pr = parse_anno(pr_anno)
            gt = parse_anno_gt(gt_anno)
...
def parse_anno_gt(filename, formal=True):
    ...
    for lane in lines['lane_lines']:
        ...
        coords = []
        for idx in range(len(lane[0])):
            coords.append((float(lane[0][idx]), float(lane[1][idx])))
        if len(coords) > 3:
            annos.append(coords)
    if formal: # true
        annos = convert_coords_formal(annos)
